...
void
mixhash(bbstate* st, const uint8_t* src, size_t src_len)
{
    blake2b_state ctx;
...
void
bb_pairing_req_build(bbstate* st, uint8_t* msgbuf)
{
...
void
bb_session_start_build(bbstate* st, struct bb_session_start_req* msg)
{
    memset(msg->reserved, 0, sizeof(msg->reserved));
    msg->counter = st->counter;
